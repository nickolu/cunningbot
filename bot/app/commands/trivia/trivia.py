...
def create_batch_question_embed(
    ...
    Args:
        ...
        batch_id: Batch game identifier
        category: Category for the batch (shared across questions)
        ends_at: When the batch ends
        stats: Optional dict with per-question stats {q_num: {correct, incorrect}}
    ...
    async def post_now(
        ...
        try:
            ...
            if method == "OpenTrivia":
                ...
                logger.info(f"Saved batch game state for batch_id {batch_id[:8]}")
                ...
                if len(opentdb_questions) > 0:
                    msg_parts.append(f"• OpenTDB: {easy_count} easy, {medium_count} medium, {hard_count} hard")
                if len(ai_questions) > 0:
                    msg_parts.append(f"• AI questions: {len(ai_questions)}")

                msg_parts.extend([
                    f"• Answer window: {answer_window_minutes} minutes",
                    f"• Batch ID: {batch_id[:8]}"
                ])
                ...
                await store.create_game(str(interaction.guild_id), batch_id, batch_data)
...
async def submit_trivia_answer_context_menu(
    ...
    # Extract game_id/batch_id from footer
